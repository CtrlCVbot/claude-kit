...
| [11_work-breakdown-structure.md](./11_work-breakdown-structure.md) | 완료 | WBS 4계층(Epic/Feature/Story/Task) + 카피 시나리오(A/B/C) 분류 |
| [12_pipeline-integration-diagram.md](./12_pipeline-integration-diagram.md) | 완료 | Mermaid 파이프라인 순서도 + 시나리오별 분기 + 진입 조건표 |
| [13_pipeline-order-analysis.md](./13_pipeline-order-analysis.md) | 완료 | 시나리오별 파이프라인 순서 분석 + Feature 유형(copy/dev) 라우팅 |
...
| 13 | [11_work-breakdown-structure.md](./11_work-breakdown-structure.md) | WBS 계층과 카피 시나리오 분류를 확인한다. |
| 14 | [12_pipeline-integration-diagram.md](./12_pipeline-integration-diagram.md) | 시나리오별 파이프라인 흐름도를 시각적으로 확인한다. |
| 15 | [13_pipeline-order-analysis.md](./13_pipeline-order-analysis.md) | 파이프라인 순서와 Feature 유형 라우팅 근거를 확인한다. |
...
| 시나리오 적응형 파이프라인 | 카피 시나리오(A: 백지, B: 부분, C: 충실도 교정)에 따라 PRD/갭분석 순서가 달라진다. A/B: PRD 먼저, C: 갭 먼저 (CAI-11/13 참조). |
| Feature 유형 라우팅 | Feature를 copy(원본 차이 닫기)와 dev(비시각적/신규 기능)로 분류하여 파이프라인 경로를 결정한다. `/plan-draft` 시점 판정 (CAI-06 §3.2 참조). |
...
| CAI-11-01 | `11_work-breakdown-structure.md` | 완료 |
| CAI-12-01 | `12_pipeline-integration-diagram.md` | 완료 |
| CAI-13-01 | `13_pipeline-order-analysis.md` | 완료 |
| CAI-IMPROVE-01 | CAI-00~10 시나리오/WBS/Feature유형 역반영 | 완료 |
