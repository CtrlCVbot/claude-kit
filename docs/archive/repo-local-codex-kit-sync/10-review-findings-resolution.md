...
> **범위**: review findings 반영 계획 문서. 이 문서는 현재 archive 상태이며, 실제 source, emitter, registry, generated output 변경은 별도 구현 이력과 대조한다.
...
| 구분 | 파일 | 처리 |
|------|------|------|
| 새 문서 | `docs/archive/repo-local-codex-kit-sync/10-review-findings-resolution.md` | Finding 1~9의 결정, 후속 구현 파일, 검증 기준을 기록 |
| 기존 문서 | `docs/archive/repo-local-codex-kit-sync/README.md` | 읽는 순서에 이 문서를 추가 |

현재 이 패키지는 `docs/archive/repo-local-codex-kit-sync/`에 있습니다. 후속 구현자는 archived plan을 그대로 active path로 가정하지 말고, 현재 source 코드와 새 active plan 문서를 먼저 확인해야 합니다.

이번 문서화에서는 아래 파일을 직접 수정하지 않습니다. 이 파일들은 후속 구현 단계의 영향 대상입니다.

| 파일 또는 경로 | 후속 구현에서의 역할 |
|------|------|
| `scripts/setup.js` | Codex direct-use output emitter, strict `src/codex` source policy, managed marker, dry-run preview 구현 |
| `src/pairing-registry.json` | `pairing-registry-v2`는 이미 적용됨. 후속 대상은 v2 audit/report 안정화와 drift metadata 운영 |
...
| `.claude/skills/kit-validation/references/schema-pairing-registry.md` | v2 schema 문서가 존재함. 후속 대상은 validator와 report 출력이 schema를 일관되게 따르는지 검증 |
| `src/claude/_meta/codex-portability.json` | hook `metadata-drift` 해소 대상 |
| `src/templates/AGENTS.md.template` | rules inline guidance와 source-link cleanup 반영 대상 |
...
| 8. registry/status schema migration 미완성 | `pairing-registry-v2`는 이미 적용됨. 남은 작업은 persistent `status`와 analyze/report `driftStatus`가 실제 audit/report 흐름에서 일관되게 쓰이는지 안정화하는 것이다. | `08-source-parity-contract.md`, `07-implementation-roadmap.md` | `src/pairing-registry.json`, `.claude/skills/kit-validation/references/schema-pairing-registry.md`, audit/report scripts | 없음 | v2 필드가 unknown field로 취급되지 않고, report가 `status`와 `driftStatus`를 혼동하지 않아야 함 |
| 9. Codex `.rules` 경로와 생성 정책 혼동 | v1에서는 `.rules` 자동 생성을 보류한다. 일반 guidance는 `AGENTS.md.template` inline guidance로 보존하고, 실제 생성하지 않는 `docs/codex-guidance/**` 링크는 쓰지 않는다. | `03-rules-index-strategy.md` | `src/templates/AGENTS.md.template`, future runtime-visible docs contract | 없음. `.codex/rules`, `~/.codex/rules`, Team Config `rules/*.rules`, `docs/codex-guidance/**` 자동 생성 금지 | `.rules` output과 존재하지 않는 guidance 링크가 dry-run 또는 install output에 포함되지 않아야 함 |

## 3. 후속 구현 영향 파일 요약

| 구현 묶음 | 파일 후보 | 관련 Finding |
|------|------|------|
| command 전환 상태와 registry v2 안정화 | `src/pairing-registry.json`, `.claude/skills/kit-validation/references/schema-pairing-registry.md`, audit/report scripts | 1, 8 |
...
| rules fallback-first routing | `src/templates/AGENTS.md.template`, future runtime-visible docs contract | 9 |
...
- generated `AGENTS.md`는 `src/claude/**`, `src/codex/**`, 존재하지 않는 `docs/codex-guidance/**` 링크를 포함하면 실패입니다.

## 5. 검증 명령 후보

- 문서 링크 검증: `docs/archive/repo-local-codex-kit-sync/*.md`의 로컬 링크 존재 확인
