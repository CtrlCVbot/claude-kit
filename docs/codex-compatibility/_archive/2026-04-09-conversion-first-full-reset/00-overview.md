...
> `src/claude`와 `src/codex`를 분리한 기준 위에서, 먼저 기존 Claude 자산을 Codex로 전환하는 migration-first 흐름을 설명하는 운영 문서 세트의 진입점.
...
- 1차 목표는 새 기능 생성이 아니라 기존 `src/claude` 자산의 Codex 전환이다.
- Codex installer는 Claude `agents/commands`를 직접 해석해 변환하는 역할이 아니다.
- `src/codex`는 hand-authored only가 아니라 conversion-generated 후 수동 보정 가능한 authoring source다.
...
이 구조의 핵심은 "설치 시 똑똑하게 변환"이 아니라 "기존 Claude source를 읽어 Codex source를 먼저 만들고, installer는 그 결과를 소비"하는 것이다.
...
현재 활성 `08~15` 문서의 1차 목표는 위 규칙을 create-time에 강제하는 것이 아니라, 이미 존재하는 `src/claude` 자산을 읽어 migration pipeline으로 `src/codex`를 채우는 것이다.
...
- 기존 `src/claude`를 어떤 규칙으로 `src/codex`로 전환해야 하는가
- 어떤 자산이 Codex sibling을 반드시 가져야 하는가
- Claude/Codex installer는 각각 어떤 source를 읽고 무엇을 읽지 않아야 하는가
- migration이 끝난 뒤 새 에이전트 기능은 어떤 문서 계약과 pairing 규칙을 따라야 하는가
...
8. [08-claude-feature-baseline.md](./08-claude-feature-baseline.md)
9. [09-guide-alignment-audit.md](./09-guide-alignment-audit.md)
10. [10-claude-to-codex-surface-mapping.md](./10-claude-to-codex-surface-mapping.md)
11. [11-codex-sibling-design-catalog.md](./11-codex-sibling-design-catalog.md)
12. [12-claude-codex-diff-matrix.md](./12-claude-codex-diff-matrix.md)
13. [13-conversion-tooling-requirements.md](./13-conversion-tooling-requirements.md)
14. [14-conversion-workflow-and-roadmap.md](./14-conversion-workflow-and-roadmap.md)
15. [15-pipeline-diagrams.md](./15-pipeline-diagrams.md)
