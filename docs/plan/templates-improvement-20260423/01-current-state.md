...
## 3.5. 개선 후 재측정 (2026-04-24, 17 TASK 완료 후)

### 3.5-1. 축별 평균 점수 변화

| 축 | Before | After | Δ | 주 개선 TASK |
|---|:---:|:---:|:---:|---|
| 1. 일관성 | 4.4 | **4.9** | +0.5 | T-04 / T-05 / T-07 |
| 2. 완결성 | 4.5 | **4.9** | +0.4 | T-02 / T-06 / T-12 |
| 3. 중복도 | 4.1 | **4.8** | +0.7 | T-03 / T-10 / T-16 |
| 4. 재사용성 | 4.2 | **4.7** | +0.5 | T-02 (merger) / T-16 |
| 5. SSOT | 4.4 | **4.9** | +0.5 | T-01 / T-15 |
| 6. 최신성 | 4.5 | **5.0** | +0.5 | 전체 TASK (v2.5.0 반영) |
| 7. 문서화 | 3.8 | **4.5** | +0.7 | T-05 / T-12 |

### 3.5-2. Location 별 평균 점수 변화

| Location | Before | After | Δ |
|:---:|:---:|:---:|:---:|
| A (`src/templates/`) | 4.36 | **4.85** | +0.49 |
| B (`plan-epic-workflow/templates/`) | 4.20 | **4.70** | +0.50 |
| C (`kit-scaffolding/references/`) | 4.30 | **4.80** | +0.50 |
| **전체 평균** | **4.30** | **≈ 4.80** | **+0.50** |

**목표 ≥ 4.60**: **초과 달성 (+0.20)** ✅

### 3.5-3. Score < 3 셀 해소 매트릭스

| 기존 위반 셀 | 점수 전환 | 해소 TASK |
|---|:---:|:---:|
| `CLAUDE.md.template` 완결성 (2) | 2 → **4** | T-02 (블록 구조 확장) |
| `AGENTS.md.template` 중복도 (1) | 1 → **5** | T-03 + T-02 |
| `AGENTS.md.template` 재사용성 (2) | 2 → **5** | T-02 (marker merger) |
| `AGENTS.md.template` SSOT (2) | 2 → **5** | T-02 + T-03 + T-15 |
| `profile.json.template` 완결성 (2) | 2 → **4** | T-06 (`_note`) |
| `profile.json.template` 최신성 (2) | 2 → **4** | T-06 |
| `settings.json.template` 재사용성 (2) | 2 → **3** | T-09 (drift guard) |
| `settings.json.template` SSOT (2) | 2 → **4** | T-09 |
| `claude-md/90-currentdate.md` 중복도 (2) | 2 → **4** | T-13 |
| `quickstart/blocks/11-repo-appendix.md` 재사용성 (2) | 2 → **4** | T-08 (링크 갱신) |
| `epic-brief.md` 중복도 (3) | 3 → **5** | T-10 (enum SSOT) |
| `phase-roadmap.md` 일관성 (2) | 2 → **5** | T-05 (`.template.md` 분리) |
| `implementation-hints.md` 일관성 (2) | 2 → **5** | T-05 |
| `template-skill.md` 완결성·문서화 (2/2) | 2 → 3 | T-12 (간접 영향) |
| `template-command-simple.md` 완결성·문서화 (2/2) | 2 → **4** | T-12 (재작성) |

**목표 Score < 3 셀 ≤ 3**: 현재 **1 셀** (template-skill 경계) — **초과 달성** ✅

### 3.5-4. 추가 효과 (후속 이슈 없이 해소)

- `substituteVars` 4 곳 중복 → 1 곳 (T-16): Location A·C renderer 일관성 향상
- AGENTS.md.template 91 → 7 줄 (T-02): 극적인 단순화, 유지보수성 향상
- quickstart 블록 네이밍 (05b 제거, T-07): Location A 일관성 +1
- Epic 상태 enum SSOT 참조 (T-10): Location B 중복도 평균 +0.4

### 3.5-5. 종합 판정

본 개선으로 **7 축 전체 0.4~0.7 개선**, 전체 평균 **4.30 → 4.80** (+0.50). 위반 셀 12 → 1. 목표 지표 모든 축 **초과 달성**.
...
| 2026-04-24 | §3.5 개선 후 재측정 추가 — 17 TASK 완료 후 축별 평균 +0.50, 위반 셀 12→1. 목표 (≥4.60) 초과 달성 (4.80). | Claude (메인테이너 역할) |
