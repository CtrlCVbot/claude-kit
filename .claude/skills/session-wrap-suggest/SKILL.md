...
description: |
  ...
  src/claude/core/hooks/session-wrap-suggest.js (Stop hook)의 fallback artifact로,
  Codex/Claude 양쪽에서 hook runtime parity 없이 동일 의도를 보존한다.
...
이 스킬은 `src/claude/core/hooks/session-wrap-suggest.js` (Stop hook, EX-001 paired-fallback)의 의미 보존 artifact다.

**원본 Hook의 한계**:
- Claude `~/.claude/.session-stats.json` 상태 파일 의존
...
- Codex/Claude 양쪽 환경에서 동일하게 invokable
- 호출 시점은 host runtime이 결정 (Claude는 Stop hook + 이 스킬 호출, Codex는 별도 trigger 또는 명시적 invocation)
...
2. **세션 종료 신호**: host runtime이 세션 종료를 알릴 때 (Claude Stop hook, Codex equivalent)
...
[Phase 3] 중복 방지 마커 (선택)
  └─ host runtime이 per-session 상태 저장 가능하면 마커 기록
     (Claude: tmpdir 마커, Codex: 세션 메타데이터, 둘 다 없으면 생략)
```
...
| Runtime | 호출 방식 | 상태 추적 |
|---------|----------|-----------|
| Claude | `src/claude/core/hooks/session-wrap-suggest.js` (Stop hook)이 이 스킬을 호출하거나, 사용자가 직접 호출 | `~/.claude/.session-stats.json` + tmpdir 마커 |
...
- 원본 Hook: `src/claude/core/hooks/session-wrap-suggest.js` (Claude 전용 Stop hook, 이 스킬 invoke의 한 경로)
- 실행 대상 스킬: `src/claude/core/skills/session-wrap/SKILL.md` (실제 세션 정리 파이프라인)
