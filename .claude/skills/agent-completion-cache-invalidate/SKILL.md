...
각 에이전트 파일(`src/claude/**/agents/*.md`)의 `tools:` 필드가 SSOT. Write/Edit 보유 시 write-capable. Role 서술이 아닌 **능력 기반** 분류.
...
| `Explore` | (Claude Code 기본) | (분석 전용) |
| `Plan` | (Claude Code 기본) | (계획 전용) |
...
| 환경 | 알림 방식 |
|------|----------|
| Claude Code | `agent-completion-cache-invalidate.js` 훅이 자동 systemMessage 출력 (tmpdir 마커로 세션당 에이전트별 1회 dedup) |
