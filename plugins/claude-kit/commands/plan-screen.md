...
임계값은 프로젝트별 조정 가능 (CLAUDE.md `idea-screening thresholds` 섹션).

## Workflow

1. **프레임워크 결정**: `--framework` 인자 → CLAUDE.md 기본값 → `rice` 폴백 순으로 해석
2. **대상 확인**: `backlog.md` 인덱스에서 위치 확인 → 해당 폴더의 IDEA 파일 로드
3. **에이전트 스폰**: `plan-idea-screener` 에이전트를 Task tool로 스폰 (framework 인자 전달)
...
- RICE: `src/claude/plan/_schemas/rice.schema.json`
- 5축: `src/claude/plan/_schemas/5axis.schema.json`
