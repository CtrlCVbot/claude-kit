---
name: "source-command-dev-sync-docs"
description: "prompt_plan.md, spec.md, AGENTS.md + rules/ 문서 동기화 (v7)"
---

# source-command-dev-sync-docs

Use this skill when the user asks to run the migrated source command `dev-sync-docs`.

## Command Template

# /dev-sync-docs - 문서 동기화 (v7)
...
## 5단계: AGENTS.md 동기화 (60줄 제한)

Glob 패턴으로 `AGENTS.md`를 탐색한다:
- `./AGENTS.md`
- `./.Codex/AGENTS.md`
...
AGENTS.md는 **60줄 이하**를 유지한다. 이를 초과하면 Codex가 규칙을 무시하기 시작한다.

**AGENTS.md에 허용되는 내용 (Core 정보만):**
- 프로젝트 개요 (1-3줄)
- 기술 스택 (5-10줄)
- 필수 명령어 (5-10줄)
- 핵심 디렉토리 구조 (5-8줄)
...
**AGENTS.md에 넣으면 안 되는 내용 → rules/로 이동:**
- 코딩 스타일/컨벤션 상세
- 테스트 규칙 상세
- API 설계 규칙
- 보안 체크리스트
...
1. AGENTS.md를 Read로 읽는다.
2. 줄 수를 카운트한다.
3. 변경된 코드에 따라 업데이트할 항목을 결정한다:
   - 새로운 빌드/실행 명령어 → AGENTS.md에 추가
   - 파일 구조 변경 → AGENTS.md에 반영
   - 새로운 의존성 → AGENTS.md에 기록
...
`.Codex/rules/` 디렉토리의 규칙 파일을 코드 변경에 맞게 동기화한다.
...
- AGENTS.md에서 분리해야 할 내용이 있으면 → 기존 rules 파일에 이동하거나, 없으면 알림
- **예외:** 7단계에서 AGENTS.md가 80줄을 초과하여 분리가 필수인 경우, 적절한 이름으로 새 rules 파일을 생성할 수 있다. 이 경우 paths frontmatter를 포함한다.

---

## 7단계: AGENTS.md 줄 수 검증

5단계에서 발견한 AGENTS.md 경로를 사용하여 줄 수를 최종 검증한다.
...
- `--check-only` 모드 → "적용하려면: /dev-sync-docs"
...
════════════════════════════════════════════════════════════════
  ...
  적용하려면: /dev-sync-docs (--check-only 없이 실행)
...
════════════════════════════════════════════════════════════════
  ...
  확인할 위치:
    - prompt_plan.md (프로젝트 루트, .Codex/, docs/)
    - spec.md (프로젝트 루트, docs/, .Codex/)
    - AGENTS.md (프로젝트 루트, .Codex/)
    - .Codex/rules/*.md
...
```
════════════════════════════════════════════════════════════════
  Sync Docs v7 (AGENTS.md 분리)
════════════════════════════════════════════════════════════════

  AGENTS.md가 N줄 → 60줄로 축소
