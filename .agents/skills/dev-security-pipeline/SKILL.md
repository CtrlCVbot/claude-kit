---
name: dev-security-pipeline
description: 보안 파이프라인 - CWE Top 25 + STRIDE 자동 검증
version: 2.0.0
---
<!-- kit:managed source=src/codex/dev/skills/dev-security-pipeline/SKILL.md hash=6ad9eef4953fe0c878739cd0a3e139f9b60734b419127b7e959d4f30d8ac221a -->

## Overview

보안 파이프라인 스킬은 코드 변경 시 자동으로 CWE Top 25 기반 보안 검증을 수행한다.
`/dev-handoff-verify --security`, `/dev-commit-push-pr` 실행 시 통합 동작한다.
...
`/dev-commit-push-pr` 실행 시 staged 파일 목록에서 위 패턴이 감지되면,
...
### /dev-handoff-verify (v6)

`/dev-handoff-verify` 커맨드의 검증 단계에서 보안 검사가 포함된다.
...
- Claude sibling: `src/claude/dev/skills/dev-security-pipeline/SKILL.md`
