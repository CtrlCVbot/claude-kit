---
name: "source-command-kit-analyze"
description: "Codex 자산의 Codex 전환 준비 상태를 분석합니다."
---

# source-command-kit-analyze

Use this skill when the user asks to run the migrated source command `kit-analyze`.

## Command Template

# /kit-analyze

`src/Codex/` 자산을 스캔하여 Codex 전환 준비 상태(auto/review/skip)를 분석한다.
...
4. 각 Codex 컴포넌트에 대해 다음을 결정한다:
   ...
   - `evidenceLevel`: 공식 지원 / 우회 가능 / 추정 / 검증 필요 (`src/Codex/_meta/codex-portability.json` 참조)
...
4-1. `--include-paired` 또는 기본 실행 시, `status: paired` 자산에 대해 content drift를 검사한다:
   - Codex source와 Codex source를 읽어 도메인 키워드(`copy`, `scenario`, `Feature 유형`, `시나리오`, `/copy-`, `copy-reference`, `routing-metadata`, `갭 분석`) 비대칭을 확인
   - Codex에 N>0 출현하는 키워드가 Codex에 0회 → `content-drift`
...
5. 타입별 분류 규칙:
   ...
   - **hook**: `src/Codex/_meta/codex-portability.json` 또는 `scripts/codex-hook-compat.js HOOK_PORTABILITY` 조회. 등록되지 않은 경우 default `paired-direct (auto)`
...
```
  ...
  === 드리프트 감지 (paired 자산) ===
  content-drift: X개 — Codex에 Codex에 없는 도메인 참조 발견
