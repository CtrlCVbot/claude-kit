---
name: "source-command-dev-checkpoint"
description: "작업 상태 저장/복원 (v6)"
---

# source-command-dev-checkpoint

Use this skill when the user asks to run the migrated source command `dev-checkpoint`.

## Command Template

# /dev-checkpoint - 작업 상태 저장/복원 (v6)
...
# staged 변경사항
git diff --cached > .Codex/checkpoints/{name}/staged.patch

# unstaged 변경사항
git diff > .Codex/checkpoints/{name}/unstaged.patch
...
```bash
mkdir -p .Codex/checkpoints/{name}/untracked
...
- `/dev-commit-push-pr` 실행 전
- `/dev-handoff-verify` 검증 3회 실패 시
...
```bash
ls .Codex/checkpoints/{name}/metadata.json
```
...
```bash
# staged 변경사항 복원
git apply .Codex/checkpoints/{name}/staged.patch
git add -A

# unstaged 변경사항 복원
git apply .Codex/checkpoints/{name}/unstaged.patch

# untracked 파일 복원
cp -r .Codex/checkpoints/{name}/untracked/* ./ 2>/dev/null
```
...
════════════════════════════════════════════════════════════════
  ...
  복원: /dev-checkpoint restore "이름"
  비교: /dev-checkpoint diff "이름"
  삭제: /dev-checkpoint delete "이름"
