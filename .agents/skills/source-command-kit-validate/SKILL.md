---
name: "source-command-kit-validate"
description: "Codex-kit 컴포넌트의 표준 준수 여부를 검증합니다."
---

# source-command-kit-validate

Use this skill when the user asks to run the migrated source command `kit-validate`.
...
| `--verbose` | 상세 출력 | 꺼짐 |
| `--target` | 타깃 플랫폼 (`Codex` / `codex`) | `Codex` |

주의: /kit-validate는 authoring source만 검증한다. `.codex/agents/*.toml` 등 runtime artifact는 대상 아님.
주의: `kit`은 검증 domain이 아니다. `.Codex/commands/kit-*`와 `.Codex/skills/kit-*`는 maintenance toolchain이며 product source parity 대상에서 제외한다.
...
1. `--target`에 따라 스캔 대상을 결정한다:
   - `--target Codex` (기본): `src/Codex/` 스캔, `schema-{type}.md` 사용
...
4. 타입에 맞는 `.Codex/skills/kit-validation/references/schema-{type}.md`를 참조한다.
