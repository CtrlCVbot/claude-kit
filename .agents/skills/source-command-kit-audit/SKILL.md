---
name: "source-command-kit-audit"
description: "Codex-kit 전체 컴포넌트의 일관성을 전수 감사합니다."
---

# source-command-kit-audit

Use this skill when the user asks to run the migrated source command `kit-audit`.
...
> 참조: `.Codex/skills/kit-validation/SKILL.md`
...
- 디렉토리 구조가 `src/Codex/{domain}/{category}/` 패턴 준수
...
- **exception-registry ↔ pairing-registry cross-check** (codex-sync Phase 4, [03-sync-pipeline-design.md §7.1](../../docs/archive/2026-04-17/codex-sync/03-sync-pipeline-design.md#71-vocabulary-mapping-exception-registry--pairing-registry) vocabulary mapping):
  ...
  - exception `strategy=paired-fallback` + `fallbackTarget=skill` → fallback artifact (`src/Codex/{domain}/skills/{component}/SKILL.md`) 존재 (WARN: artifact 무결성)
...
- **S3 감지** (skill fallback artifact 삭제, 예: EX-001):
  ...
  node -e "
    ...
    for (const e of d.entries) {
      if (e.strategy==='paired-fallback' && e.fallbackTarget==='skill' && e.status==='resolved') {
        const artifactPath = 'src/Codex/' + (e.domain || 'core') + '/skills/' + e.component + '/SKILL.md';
  ...
  "
  ```
- **S4 감지** (pairing-registry enum value 오류): `.Codex/skills/kit-validation/references/schema-pairing-registry.md` 참조 (codex-sync cross-phase review CC5, Phase 5+ 신규)
...
- **rule fallback drift**: `src/Codex/core/rules/{name}.md` 변경 뒤 대응 `src/templates/agents-md/*.md` block이 갱신되지 않은 경우 감지 (INFO)
  - 검사: 일반 rule은 두 파일의 git log 비교. EX-009 `security-no-hardcoded-secrets`는 partial source라 파일 단위 시간 비교에서 제외
- **hook fallback drift**: `src/Codex/{domain}/hooks/{name}.js` 변경 시 fallback skill (`src/Codex/{domain}/skills/{name}/SKILL.md`) 의도 정합성 (INFO)
...
- **paired-direct sibling drift** (시간 기반): `claudeSource`와 `codexSource`가 모두 존재하는 항목의 수정일 차이 > 7일 (INFO)
  - 예: EX-002 output-secret-filter의 Codex/Codex 버전 분기 일관성. 단, `pairing-registry.json`의 `contentHash`가 현재 Codex source hash와 일치하거나 `lastSyncedAt`이 양쪽 source 변경보다 최신이면 동기화 완료로 간주
- **paired-content-drift** (내용 기반, `--content` 플래그): Codex source 본문에 존재하는 도메인 키워드(`scenario`, `Feature 유형`, `/copy-`, `copy-reference`, `갭 분석` 등)가 Codex source 본문에 없는 비대칭 감지 (WARN)
  ...
  - 예: copy 도메인 도입으로 수정된 plan-draft, dev-feature 등 기존 컴포넌트의 Codex↔Codex 내용 불일치
...
- G2: src/Codex/ 자산 중 src/codex/ 대응 없는 required sibling (WARN)
- G3: docs/archive/2026-04-17/meta-tooling/ 명세에 정의됐지만 .Codex/에 없는 도구 (WARN)
...
1. `src/Codex/` 전체를 스캔하여 컴포넌트 인벤토리를 구축한다.
