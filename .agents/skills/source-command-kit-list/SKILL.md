---
name: "source-command-kit-list"
description: "Codex-kit의 모든 컴포넌트를 도메인/타입별로 조회합니다."
---

# source-command-kit-list

Use this skill when the user asks to run the migrated source command `kit-list`.

## Command Template

# /kit-list

`src/Codex/` (및 `src/codex/`) 소스 트리를 스캔하여 전체 컴포넌트 현황을 출력한다.
...
| `--target` | 타깃 필터 (`Codex`/`codex`/`both`) | `both` |
...
1. `src/Codex/` 하위를 스캔한다:
   - `src/Codex/{domain}/skills/` → `SKILL.md`가 있는 디렉토리
   - `src/Codex/{domain}/agents/` → `*.md` 파일
   - `src/Codex/{domain}/commands/` → `*.md` 파일
   - `src/Codex/{domain}/hooks/` → `*.js` 파일 (`package.json` 제외)
   - `src/Codex/core/rules/` → `*.md` 파일
