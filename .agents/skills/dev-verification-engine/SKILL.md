---
name: dev-verification-engine
description: 통합 검증 엔진 - 서브에이전트 기반 fresh-context 검증 루프 (v6)
version: 2.0.0
---
<!-- kit:managed source=src/codex/dev/skills/dev-verification-engine/SKILL.md hash=d3a0b2db8b57c4aa1135bb46e59fff08c497ec47f030cb85f9ec77fb2bfdb317 -->
...
통합 검증 엔진 스킬은 `/dev-handoff-verify` 커맨드의 핵심 동작을 정의한다.
v5의 `/dev-test-verify` 단독 커맨드에서 v6의 `/dev-handoff-verify` 통합 커맨드로 진화하였다.
...
| 트리거 | 설명 |
|--------|------|
| `/dev-handoff-verify` | 핸드오프 + 기본 루프 검증 (최대 5회) |
| `/dev-handoff-verify --once` | 핸드오프 + 단발 검증 |
| `/dev-handoff-verify --loop 3` | 핸드오프 + 3회 루프 |
| `/dev-handoff-verify --security` | 핸드오프 + 보안 검증 포함 |
| `/dev-handoff-verify --coverage` | 핸드오프 + 테스트 커버리지 분석 |
| `/dev-handoff-verify --extract` | 핸드오프 + 에러 추출/분류 모드 |
| `/dev-handoff-verify --skip-handoff` | 핸드오프 생략 + 검증만 |

### 자동 호출

| 상황 | 동작 |
|------|------|
| `/dev-commit-push-pr` 실행 전 | `--once` 모드로 사전 검증 |
...
```
/dev-handoff-verify 커맨드 (부모 컨텍스트)
   │
   ├── [1] handoff.md 자동 생성
...
| v5 | v6 |
|----|-----|
| /handoff → /clear → /dev-test-verify | /dev-handoff-verify (단일 커맨드) |
...
`/dev-handoff-verify --coverage` 실행 시 테스트 커버리지를 분석한다.
...
`/dev-handoff-verify --extract` 실행 시 에러를 추출하고 분류한다.
...
### /dev-handoff-verify 커맨드

이 스킬은 `/dev-handoff-verify` 커맨드의 핵심 로직을 정의한다.
커맨드 파일(`commands/dev-handoff-verify.md`)이 이 스킬의 파이프라인을 실행한다.
...
5회 루프 실패 시 `/dev-learn --from-error`를 자동 제안하여
에러 패턴을 CLAUDE.md에 기록한다.

### /dev-commit-push-pr 커맨드
...
- Claude sibling: `src/claude/dev/skills/dev-verification-engine/SKILL.md`
