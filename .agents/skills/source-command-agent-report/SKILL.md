---
name: "source-command-agent-report"
description: "에이전트 호출 텔레메트리 리포트. ~/.Codex/logs/agent-telemetry.jsonl을 집계하여 에이전트별 호출 빈도·성공률·응답 시간을 표 형태로 출력한다. IMP-AGENT-009."
---

# source-command-agent-report

Use this skill when the user asks to run the migrated source command `agent-report`.
...
1. **로그 파일 로드**: `~/.Codex/logs/agent-telemetry.jsonl` 읽기
...
- **원격 전송 없음**: 로그는 `~/.Codex/logs/` 로컬 전용
- **PII 마스킹**: `metadata` 필드의 PII 마스킹 책임은 이벤트 emit 에이전트 측

## 관련

- 스키마: `src/Codex/core/_schemas/agent-telemetry.schema.json`
- 룰: `.Codex/rules/agent-telemetry.md`
- 훅 (미구현, 별도 세션): `src/Codex/core/hooks/agent-telemetry-emit.js`
