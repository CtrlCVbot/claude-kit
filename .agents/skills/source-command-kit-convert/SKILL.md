---
name: "source-command-kit-convert"
description: "Codex 자산을 Codex 형식으로 전환합니다."
---

# source-command-kit-convert

Use this skill when the user asks to run the migrated source command `kit-convert`.

## Command Template

# /kit-convert

Codex 자산을 Codex 형식으로 변환하여 `src/codex/`에 생성하고 pairing-registry를 갱신한다. command는 항상 command-only로 변환하지 않고 `pairing-registry-v2`의 transition state를 따른다.

> 참조: `.Codex/skills/kit-converter/SKILL.md`
...
[kit-convert --dry-run] 변환 계획
  ...
  |-----------------|---------|------------------------------------------|------------------------------------------|------------|
  | dev-architect   | agent   | src/Codex/dev/agents/dev-architect.md   | src/codex/dev/agents/dev-architect.md    | auto       |
  ...
```

### Phase 3: 변환 실행

6. 타입별 변환 규칙을 적용한다 (`kit-converter/references/conversion-rules.md` 참조). codex-sync Phase 4 4-tier strategy를 `src/Codex/_meta/codex-portability.json`에서 조회:
   ...
   - **Hook** (paired-direct / paired-fallback): codex-portability.json strategy 분기:
     - paired-direct: setup.js T18으로 src/codex/ 우선, 없으면 src/Codex/ JS 복사 + Codex 등록 주석
...
10. 동기화 메타데이터를 기록한다 (초기 변환 및 `--force` 재변환 모두 적용):
    ...
    - `contentHash`: Codex source 파일의 raw UTF-8 내용을 SHA-256 해싱 후 앞 8자 hex (`require('crypto').createHash('sha256').update(content,'utf8').digest('hex').slice(0,8)`)
...
- `.Codex/commands/kit-*`, `.Codex/agents/kit-sync-agent.md`, `.Codex/skills/kit-*`는 product migration 대상이 아니다.
- `src/codex/kit/**`와 `src/Codex/kit/**`는 생성하지 않는다.
