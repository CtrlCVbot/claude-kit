'use strict';
/**
 * copy-collector.js — kit-feedback-archiving Phase 3.2.3
 *
 * 7개 copy 커맨드별 copy_specific 필드 추출.
 * Claude peer/Codex sibling: src/codex/core/collectors/copy-collector.js
 * 설계: docs/plan/kit-feedback-archiving/03-trigger-points.md §3.2
 */
...
function _parseGapSummary(text, prefix) {
  ...
  const m = text.match(re);
  if (!m) return null;
  return { P0: parseInt(m[1], 10), P1: parseInt(m[2], 10), P2: parseInt(m[3], 10) };
}
...
function _collectReferenceRefresh(t) {
  ...
  return {
    ...
    evidence_status: evidence ? {
      valid: parseInt(evidence[1], 10),
      stale: parseInt(evidence[2], 10),
      missing: parseInt(evidence[3], 10)
    } : null
  };
}

function _collectVisualReview(t) {
  return { gap_summary: _parseGapSummary(t, 'Visual\\s+gaps') };
}

function _collectInteractionReview(t) {
  return { gap_summary: _parseGapSummary(t, 'Interaction\\s+gaps') };
}

function _collectGapBoard(t) {
  // 통합 gap 총계 + priority 분포
  const gapSummary = _parseGapSummary(t, 'Priority\\s+distribution');
  return {
...
function _collectPlanUnit(t) {
  return { execution_units: _num(t, /Execution\s+Units?\s+created:\s*(\d+)/i, 1) };
}

function _collectVerify(t) {
  return { verify_result: _firstMatch(t, /Verify\s+result:\s*(PASS|WARN|FAIL)/i, 1) };
}

function _collectCloseout(t) {
  return { gap_resolution_rate: _num(t, /Gap\s+resolution\s+rate:\s*(\d+)\s*%/i, 1) };
}
