'use strict';
/**
 * rollup-generator.js — kit-feedback-archiving Phase 3.3.3
 * 설계: docs/plan/kit-feedback-archiving/04-archive-layout.md §5
 */

const { buildStatsFromEntries } = require('./stats-aggregator.js');

/**
 * entry_id 앞 8자리(YYYYMMDD)에서 YYYY-MM 추출.
 */
...
function renderRollupMd(rollup) {
  ...
  const domainLines = Object.entries(domain)
    .filter(([, c]) => c > 0)
    .map(([name, c]) => `- ${name}: ${c}건 (${Math.round(c * 100 / (total || 1))}%)`)
    .join('\n') || '- —';

  const issueLines = issues.slice(0, 5)
    .map((i, idx) => `${idx + 1}. ${i.type} (${i.count}건)`)
    .join('\n') || '- —';

  const impKitLines = Object.entries(impKit)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
    .map(([id, c]) => `- ${id}: ${c}건`)
    .join('\n') || '- —';
