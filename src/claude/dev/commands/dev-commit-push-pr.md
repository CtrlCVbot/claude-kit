...
2. 재시도: /dev-commit-push-pr --merge
```
→ 중단
...
다음 단계:
  PR 리뷰 후: /dev-commit-push-pr --merge --notify
  /dev-sync-docs
════════════════════════════════════════════════════════════════
```
...
계속 작업:
  [코드 수정] → /dev-commit-push-pr --draft
════════════════════════════════════════════════════════════════
```
...
# 머지 + MCP 알림 (v6 핵심 플로우)
/dev-commit-push-pr --merge --notify

# 머지하되 체크리스트 스킵 (docs만 변경 등)
/dev-commit-push-pr --merge --no-checklist

# Squash 머지 (커밋 정리) + 알림
/dev-commit-push-pr --squash --notify

# Rebase 머지 (히스토리 유지)
/dev-commit-push-pr feat: 로그인 기능 --rebase

# Draft PR (WIP)
/dev-commit-push-pr --draft

# 긴급 수정 + 즉시 머지 + 모든 검증 스킵 + 알림
/dev-commit-push-pr fix: hotfix --merge --no-verify --no-checklist --skip-security --notify

# 보안 검증만 스킵 (빌드/테스트는 유지)
/dev-commit-push-pr --merge --skip-security
```
