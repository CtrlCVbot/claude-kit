...
# /dev-handoff-verify - 핸드오프 + 자동 검증 통합 (v6)

v5의 `/handoff` + `/clear` + `/dev-test-verify`를 **하나의 커맨드**로 통합.
...
| v5 | v6 |
|----|-----|
| /handoff → /clear → /dev-test-verify (3단계, 수동) | /dev-handoff-verify (1단계, 자동) |
...
다음 단계:
  - /dev-handoff-verify (자동 수정 포함 루프 검증)
  - /dev-handoff-verify --effort max (최대 깊이 분석)
════════════════════════════════════════════════════════════════
```
...
────────────────────────────────────
다음 단계: /dev-handoff-verify --effort high (수정 후 재검증)
════════════════════════════════════════════════════════════════
```
...
권장: /dev-handoff-verify (루프 모드로 자동 수정 시도)
또는: /dev-handoff-verify --extract (에러 분류 상세 확인)
════════════════════════════════════════════════════════════════
```
...
권장 조치:
  1. /dev-learn --from-error로 교훈 기록
  2. --effort max로 재시도 (현재보다 높은 effort인 경우)
  3. 수동 수정 후 /dev-handoff-verify --skip-handoff
════════════════════════════════════════════════════════════════
```
...
/dev-handoff-verify "결제 로직 리팩토링 검증"

# 단발 검증
/dev-handoff-verify --once "빠른 빌드 확인"

# 최대 깊이 + 보안 포함
/dev-handoff-verify --effort max --security "인증 모듈 변경"

# 에러 추출만
/dev-handoff-verify --extract

# 커버리지 분석
/dev-handoff-verify --coverage

# 루프 3회 + 린트만
/dev-handoff-verify --loop 3 --only lint

# 낮은 effort (빠른 확인)
/dev-handoff-verify --effort low --once

# handoff.md 이미 있을 때 (재검증)
/dev-handoff-verify --skip-handoff
...
- 부모 컨텍스트가 보존되므로 검증 후 바로 /dev-commit-push-pr 진행 가능.
