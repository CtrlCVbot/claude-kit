...
# dev-checkpoint — Codex Entry Flow

## Overview

작업 상태 저장/복원 (v6). 체크포인트를 생성하고 복원하며, 저장된 상태를 조회·비교·삭제하는 도구다.

## Invocation

```
dev-checkpoint save|restore|list|diff|delete ["이름"] [--tag 태그]
```
...
위 Parameters 표에 따라 서브커맨드와 옵션을 파싱한다.
...
- Claude sibling: src/claude/dev/commands/dev-checkpoint.md
