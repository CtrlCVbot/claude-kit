...
<!-- REVIEW NEEDED: write-capable agent with Bash -->
<!-- Claude sibling: src/claude/dev/agents/dev-implementer.md -->
---
name: dev-implementer
description: Feature Package TASK의 TDD Red-Green 루프를 자율 실행하는 구현 에이전트. /dev-run 커맨드가 TASK별로 호출. handoff-contract.json의 file_scope·task_ids 소비.
...
release_stage: beta
dependencies: 
  calls: ["dev-architect","dev-code-reviewer","dev-database-reviewer","dev-security-reviewer"]
  called_by: ["copy-implementer","dev-architect","dev-code-reviewer","dev-database-reviewer","plan-bridge-writer"]
---
<Agent_Prompt>
  <Role>
  ...
  <Input_Format>
    **필수 입력** (메인 세션이 프롬프트에 명시):
    - TASK ID (예: `T-PREVIEW-03`)
    - handoff-contract.json 경로 또는 내용
    - Feature Package 경로

    **권장 입력**:
    - 이전 TASK 완료 상태 (의존성 있는 경우)
    - 관련 REQ-ID 리스트
  </Input_Format>
