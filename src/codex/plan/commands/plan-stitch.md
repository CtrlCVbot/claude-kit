...
```
plan-stitch {slug}
plan-stitch {slug} --force-sequential --sequential-reason "이유"
```

## Flags (IMP-KIT-027)

- `--force-sequential` — routing-metadata의 `post_wireframe_path`가 이미 `design`인 경우에도 실행
...
3. **에이전트 스폰**: `plan-stitch-integrator` 에이전트 호출
   - PRD ↔ Wireframe 일관성 검증
   - 요구사항-화면 매핑 (REQ-ID → Screen-ID)
   - 누락 탐지 및 보고
   - 통합 컨텍스트 문서 생성
4. **routing-metadata 갱신**:
   - `post_wireframe_path` 필드 설정 (`stitch` 또는 `design+stitch`)
