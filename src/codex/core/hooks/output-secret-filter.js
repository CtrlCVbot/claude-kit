<!-- kit-convert generated: 2026-04-10 -->
/**
 * output-secret-filter.js — PostToolUse 훅 (전체 도구)
 * 도구 출력에서 비밀값(API 키, 토큰, 비밀번호)을 마스킹한다.
 * Codex 호환 버전: CODEX_SANDBOX 또는 CLAUDE_REMOTE_SESSION 환경변수로 활성화.
 * 차단하지 않음 (항상 exit 0).
 *
...
function main() {
  ...
  process.stdin.on('end', () => {
    try {
      // Codex/Claude 원격 세션에서만 활성화
      const isRemote = process.env.CODEX_SANDBOX || process.env.CLAUDE_REMOTE_SESSION;
      if (!isRemote) {
        process.exit(0);
      }
      ...
      if (maskCount > 0) {
        ...
        // Log (Codex 환경: .codex/logs/, Claude 환경: ~/.claude/)
