'use strict';
/**
 * index-renderer.js — kit-feedback-archiving Phase 3.3.2
 * 설계: docs/plan/kit-feedback-archiving/04-archive-layout.md §3
 */
...
function _renderLatestRows(ids) {
  ...
  return ids.map((id) => {
    // entry_id 형식: YYYYMMDD-HHmmss-command-suffix-slug
    const m = id.match(/^(\d{4})(\d{2})(\d{2})-(\d{2})(\d{2})\d{2}-(.+?)-(.+)$/);
    if (!m) return `| ? | ? | ? | ? | ? | 0 | [link](./${id}.json) |`;
    const [, y, mo, d, hh, mm, cmd, slug] = m;
    return `| ${y}-${mo}-${d} ${hh}:${mm} | claude | — | /${cmd} | ${slug} | 0 | [link](./${id}.json) |`;
  }).join('\n');
}

/**
 * stats.json → index.md 마크다운 렌더링.
 */
