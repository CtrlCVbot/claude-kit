...
### Added — KIT 이관 dev 스킬 2건 (2026-04-28)

전역 `~/.claude/skills/` 의 단일 스킬 2건을 claude-kit dev 도메인으로 이관 + 책임 구분 표 추가:
...
- **Skill (IMP-AGENT-017)**: `src/claude/dev/skills/dev-verify-pattern-runner/` — 프로젝트의 모든 `verify-*` 스킬을 통합 실행하는 메타 오케스트레이터
  - `SKILL.md` (1 파일, 6단계 워크플로우 + 적응형 실행 ≤5순차/≥6병렬 + 4 플래그 --fix/--report-only/--severity/--category)
  - `disable-model-invocation: true` (명시적 호출만)
  - dev-verification-engine (기술 검증 - 빌드/타입/린트/테스트) 과 책임 구분 표 명시 → 상호보완
  - `/dev-verify-fe` / `/dev-verify-all` (테스트 품질) 와도 책임 구분 표

본 이관은 위 §"KIT 후보 5건 중 책임 분리 발견 2건" 의 처리 완결 (IMP-AGENT-016/017). KIT 이관 5건 모두 완료.
